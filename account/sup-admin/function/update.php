...
if (isset($_GET['update_active_staff'])) {
    ...
    if (isset($_GET['user_id'], $_GET['status'])) {
        ...
        if ($stmt->affected_rows > 0) {
            $_SESSION['success'] = "Successfully Activated";
            // Log admin activity
            updateActivity_user_admin("Update Active", "Admin Account: $user_id successfully activated", $user_id, $conn);
        } else {
            // Session for failure
            $_SESSION['failed'] = "Failed to Activate";
        }
...
if (isset($_GET['update_highlight_yes'])) {
    ...
    // Construct the query to select status for the service
    $status_query = "SELECT status FROM service WHERE service_id = '$id'";
    ...
    if ($status_result) {
        ...
        // Construct the query for updating highlight
        $active_query = "UPDATE service SET highlight = '$highlight' WHERE service_id = '$id'";
        ...
        if ($active_result) {
            $_SESSION['success'] = "Successfully Highlighted";
            // Add activity log
            seviceActivity_admin("Updated Highlight", "Updated Highlight for service ID $id to $highlight", $user_id, $conn);
        } else {
        ...
        // Error handling for status query
        $_SESSION['failed'] = "Failed to retrieve service status.";
        // Redirect back
        echo "<script>window.history.back();</script>";
        exit;
    }
...
// Check if the necessary parameters are set for updating highlight to 'Not Highlight'
if (isset($_GET['update_highlight_no'])) {
    ...
    if ($status_result) {
        ...
        // Check if the query executed successfully
        if ($active_result) {
            $_SESSION['success'] = "Successfully Not Highlighted";
            // Add activity log
            seviceActivity_admin("Updated Highlight", "Updated Highlight for service ID $id to $highlight", $user_id, $conn);
        } else {
            $_SESSION['failed'] = "Failed to change Highlight";
