                ...
                <div class="container-fluid px-4">
                    ...
                    <div class="row">
                        <div class="col-xl-6">
                            <div class="card mb-4">
                                ...
                                WHERE sc.location_id = '{$row['location_id']}' 
                                ...
                                if ($result->num_rows > 0) {
                                    while ($count_row = $result->fetch_assoc()) {
                                        $data[] = array(
                                            'service_offer' => $count_row['service_offer'],
                        ...
                        <div class="col-xl-6">
                            <div class="card mb-4">
                                <div class="card-header">
                                    ...
                                    <a href="./apk_history.php">Weekly Schedule Bar Chart</a>
                                </div>
                                <div class="card-body">
                                    <div>
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            ...
                            DataTable for List Appointment Today
                        </div>
                        <div class="card-body">
                            <table class="table table-bordered table-hover dt-responsive" id="datatablesSimple">
                                ...
                                <tbody>
                                    <?php
                                    $currentDate = date('Y-m-d'); // Changed date format to match MySQL's date format (YYYY-MM-DD)
                                    $user_query = mysqli_query($conn, "SELECT * FROM schedule WHERE date = '$currentDate' AND `location_id` = '{$row['location_id']}'") or die(mysqli_error($conn));

                                    while ($schedule_row = mysqli_fetch_array($user_query)) {
                                        $id = $schedule_row['id'];
                                        ...
                                        $member_stmt = mysqli_prepare($conn, "SELECT * FROM members WHERE member_id = ?");
                                        mysqli_stmt_bind_param($member_stmt, "i", $member_id); // Assuming member_id is an integer
                                        ...
                                        mysqli_stmt_bind_param($user_stmt, "i", $user_id); // Assuming service_id is an integer
                                        mysqli_stmt_execute($user_stmt);
                                        $user_result = mysqli_stmt_get_result($user_stmt);
                                        $user_row = mysqli_fetch_array($user_result);

                                        $service_stmt = mysqli_prepare($conn, "SELECT * FROM service WHERE service_id = ?");
                                        mysqli_stmt_bind_param($service_stmt, "i", $service_id); // Assuming service_id is an integer
