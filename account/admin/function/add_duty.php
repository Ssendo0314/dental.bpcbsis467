    ...
    <div class="container contact-form">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="user" class="form-control" required>
                            ...
                            $query = mysqli_query($conn, "SELECT * FROM users WHERE `status` = 'active' AND `location_id` = {$row['location_id']} AND user_id NOT IN (SELECT user_id FROM duty)") or die(mysqli_error($conn));
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <?php $duty_main_query = "SELECT * FROM duty WHERE user_id = '{$row['user_id']}'";
                        ...
                        while ($duty_main_row = mysqli_fetch_array($duty_main_result)) {
                            ...
                            <p>All the User Account are From this location
                               <strong><?php echo $main_row['location']; ?></strong>
                            </p>
                        <?php } ?>
                    </div>
                    <div class="form-group">
                        <input type="hidden" id="location_id" class="form-control"
                            value="<?php echo $row['location_id']; ?>" name="location_id" required>
                            <button type="submit" name="duty_edit" class="btn btn-success">Submit</button>
                        <a class="small-link" href="../duty.php?location_id=<?php echo $main_row['location_id']; ?>">Back</a>
                    </div>
                </div>
            </div>
        </form>
