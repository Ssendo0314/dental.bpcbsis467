                    ...
                    <div>
                        ...
                        while ($record_row = $result->fetch_assoc()) {
                            ...
                            <form method="post" action="./function/add.php">
                                ...
                                <input type="hidden" name="teeth_no" value="<?php echo htmlspecialchars($teeth_no); ?>" />
                                ...
                                if ($record_result->num_rows > 0) {
                                    ...
                                    while ($record01_row = $record_result->fetch_assoc()) {
                                        ...
                                        if ($record01_row['teeth_no']) {
                                            ...
                                            $schedule_update_sql = "UPDATE `schedule` SET `record_id` = ? WHERE `id` = ?";
                                            $schedule_update_stmt = $conn->prepare($schedule_update_sql);
                                            // Bind parameters for the update statement
                                            $schedule_update_stmt->bind_param("si", $record02_id, $schedule01_id);
                                            // Execute the update query
                                            $schedule_update_stmt->execute();
                                            ...
                                            $delete_stmt->bind_param("i", $schedule01_id);
                                    ...
                                    echo "<tr><td colspan='4' class='bg-primary'>No records found</td></tr>";
                                } ?>
                            </tbody>
                        </table>
                    </div>
